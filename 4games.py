...
def data_in_news(url, user_date):
    ...
    if r.status_code == rq.codes.ok:
        ...
        date_for_count = re.sub('[\u4e00-\u9fa5]', '', date)
        if(len(date_for_count) < 8):
            date_for_count = int(date_for_count[0:4] + "0" + date_for_count[4:5] + "0" + date_for_count[5:6])
        ...
        print("Your url is worng or somrthing else")
    ...
    if array != []:
        for i in range(0, len(array)):
            ...
            try:
                ...
                doc.add_paragraph("你那圖片怪怪的！")
            j += 1
            doc.add_heading('內文', level=2)
            try:
                doc.add_paragraph("\n" + array[i][5])
            except:
                doc.add_paragraph("你那文字怪怪的！")
            doc.add_page_break()
